...
**Let each studio fork the `@zuga-technologies/design-tokens` package.** Each studio maintains a copy and diverges as needed. Rejected because this is the failure mode we are explicitly preventing. Divergent forks mean no global rebrand, no consistent contrast guarantees, no shared enforcement, and version drift that compounds over time. The sub-brand profile system (§15) is the correct alternative: expressive identity within a bounded, auditable space.
