...
**blue-700 as a more accessible alternative.** A darker, clearly accessible stop in the blue family. Rejected: blue is assigned to ZugaCode (see §5.4, `acc04` in §5.10). Using blue-700 as the master brand accent would create a collision with the ZugaCode dev-tools convention — the brand accent and the developer-studio accent would be perceptually indistinguishable in mixed navigation.
